...
- **Library namespaces**: Any userscript can serve as a shared library. Reference it via `epupp://script-name.cljs` in `:epupp/inject`. Dependencies resolve transitively. Built-in shared library: `epupp://epupp/ui.cljs` (namespace `epupp.ui`, branding components). See `docs/epupp-README.md` for examples.
- **REPL bootstrap APIs**: `epupp.fs`, `epupp.repl`, and `epupp.tools` are automatically available when the REPL connects - no `:epupp/inject` needed. Just `require` the namespace.
  - `epupp.tools` provides element and viewport screenshot capture. require `[epupp.tools :as tools]`. Functions: `capture-element`, `capture-selector`, `capture-visible` - all `^:async`, return Promises with `{:success bool :dataUrl string :error string}`. See `docs/epupp-README.md` for details.
